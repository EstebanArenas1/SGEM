...
json_path='/pscratch/sd/j/jehr/MSFT/BTReport/merged_reports_btreport_V3_seg2exp_filtered.json'
...
for sub in sorted(report_dict.keys()):
    if not report_dict[sub].get("Predicted Report (Seg2Exp)"):
        continue
    ...
    text_predicted=report_dict[sub]["Predicted Report (Seg2Exp)"]
...
if __name__ == '__main__':
    ...
    for key in keys:
        model_id='llama3:70b'
        save_dir=f"/pscratch/sd/j/jehr/MSFT/BTReport/btreport/evaluation/intermediate_feature_validation/seg2exp/{model_id}/{'-'.join(key.lower().split(' '))}" ##
