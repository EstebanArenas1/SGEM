...
for sub in sorted(report_dict.keys()):
    ...
    tmp_predicted = parse_radiology_report(report_dict[sub]["Predicted Report (gpt-oss:120b)"]) ##
...
if __name__ == '__main__':
    keys=['Clinical Report']
    for key in keys:
        # model_id='gemma3:27b'
        model_id='llama3:70b'
        save_dir=f"/pscratch/sd/j/jehr/MSFT/BTReport/btreport/evaluation/intermediate_feature_validation/btreport_gptoss120b/{model_id}/{'-'.join(key.lower().split(' '))}" ##
