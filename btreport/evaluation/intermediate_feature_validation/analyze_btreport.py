...
json_path='/pscratch/sd/j/jehr/MSFT/BTReport/results/fakin/merged_reports_btreport.json'
...
for sub in sorted(report_dict.keys()):
    ...
    tmp_predicted = parse_radiology_report(report_dict[sub]["Predicted Report (gpt-oss:120b, run_name=deepmedic)"]) ##
...
if __name__ == '__main__':
    ...
    for key in keys:
        # model_id='gemma3:27b'
        model_id='llama3:70b'
        save_dir=f"/pscratch/sd/j/jehr/MSFT/BTReport/btreport/evaluation/intermediate_feature_validation/btreport_gptoss120b_deepmedic/{model_id}/{'-'.join(key.lower().split(' '))}" ##
        subjects = sorted(list(report_dict.keys()))

        split_subjects = subjects[args.split_no :: args.num_splits]
        ...
        for id in tqdm(split_subjects, total=len(split_subjects), colour='green'):
